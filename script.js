...
/*
Since I'm about to stop working on this for now, here are some notes about the current state, accomplishments, and some lessons.

TO DO:
- Finish the colour-set window.
    - back/forward buttons to move between colours schemes, which we can store in the colourScheme object as arrays. .fg and .bg then turn into functions that just grab the last ones. (I probably won't implement this in the interest of getting back to my studies.)
- Features
    - Toggle whether hour and second hands should move every second or only when they increment.
    - In colour-select modal: enter to change colours, escape to close window. (Maybe this is solved by other frameworks like Material?)
- Code Improvements
    - Change the digital text to use visibility: hidden instead of... all of that code... sigh...
    - I could probably make the "hide text" smarter, so that there are classes of things that are hide-able and I could reference them dynamically rather than hard-coding them in.
    - I am using a very informal method of constructors and objects. I could probably turn these into classes.
    - The order of my code is random. Could be sorted more into actual objects/classes and maybe they should be includes rather than all in one giant file.
    - Change tick styles so they apply classes rather than overwriting style elements.

ACCOMPLISHMENTS:
- Learned to call APIs (and parse JSON), at least in a very basic way.
- Learned some code structure lessons the hard way (if I kept going with this, there are more improvements that could be made).
- Learned about CSS animations. They are theoretically simple, but in practice take some care to do well. E.g. snapToNow() remains problematic and could probably be implemented in a smarter way.
- I learned how to do modals in CSS.
- I learned how to use flexbox in a practical way
- I learned a ton about the "this" keyword and how tricky it can be in certain cases.
- Very generally, I feel like I now have a good handle now on the relationships between HTML5, CSS, and JS.

LESSONS:
 - OMG I CAN JUST USE VISIBILITY: HIDDEN BLARGHLGKHDLGKJGLJ!!!11!1!1
    - Okay, so instead I'm now using animated opacity because it is prettier. But still, much easier than managing all the colours myself.
 - It seems way more flexible, easy, and reliable to add and remove classes to change formatting rather than directly writing to the style of the element. For changing colours, it would be better if I could update the css with JS rather than writing to the elements directly. It would also be easier if I had a limited set of known colours and could build classes for each one, but this would limit the desired feature far too much.
 - Be careful with specificity problems xD If you have properties identified by id and then try to apply classes, it'll cause problems.
 */